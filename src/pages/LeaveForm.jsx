...
function LeaveForm({ userProfile }) {
  ...
  async function fetchFlows() {
    const { data } = await supabase
      ...
      .eq('is_active', true)
    setFlows(data || [])
    const defaultFlow = data?.find(f => f.is_default)
    if (defaultFlow) setForm(prev => ({ ...prev, flow_id: defaultFlow.id }))
  }

  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.leave_type_id || !form.flow_id || !form.start_date || !form.end_date || !form.reason) {
      alert('請填寫所有欄位')
      return
    }
  ...
  if (success) return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
        <button
          onClick={() => { setSuccess(false); setForm({ leave_type_id: '', flow_id: '', start_date: '', end_date: '', reason: '' }) }}
          style={{
            padding: '10px 20px',
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <select
              value={form.flow_id}
              onChange={e => setForm(prev => ({ ...prev, flow_id: e.target.value }))}
              required
              style={inputStyle}
            >
